...
for it in range(iterations):
    ...
    keep_feat = [index for index, variances in enumerate(zip(sel0.variances_, sel1.variances_, seln.variances_)) if
                 variances[0] > 0.8 and variances[1] > 0.8 and variances[2] > 0.8]
    ...
    gbm_model_0 = __get_model(lgb_train_0, gbm.Dataset(x_val_0, y_val_0, reference=lgb_train_0), x_train_0, y_train_0,
                              params)
    lgb_train_1 = gbm.Dataset(x_train_1, y_train_1)
    gbm_model_1 = __get_model(lgb_train_1, gbm.Dataset(x_val_1, y_val_1, reference=lgb_train_1), x_train_1, y_train_1,
                              ar_params)
    lgb_train_n = gbm.Dataset(x_train_n, y_train_n)
    gbm_model_n = __get_model(lgb_train_n, gbm.Dataset(x_val_n, y_val_n, reference=lgb_train_n), x_train_n, y_train_n,
                              params)
    ...
    print('0 Round validation ROC-AUC = {}, accuracy = {}, recall = {}, precision = {}'.format(
        roc_auc_score(y_val_0, y_pred_0),
        accuracy_score(y_val_0, xg_predictions_0), recall_score(
            y_val_0, xg_predictions_0),
        precision_score(y_val_0, xg_predictions_0)))
    print('1 Round validation ROC-AUC = {}, accuracy = {}, recall = {}, precision = {}'.format(
    ...
    print('n Round validation ROC-AUC = {}, accuracy = {}, recall = {}, precision = {}'.format(
        roc_auc_score(y_val_n, y_pred_n),
